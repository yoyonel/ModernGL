...
2. Three unsigned bytes passed using ``3f1`` may be assigned to a ``vec3``
   shader variable, as one would expect. But, alternatively, they can also be
   assigned to a ``vec4`` shader variable. This is intended for passing a
   buffer of 3-byte RGB values into a shader which also handles an alpha
   channel.
