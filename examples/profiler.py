...
class Profiler(object):
    ...
    def begin(self, stage_name=''):
        ...
        for i, part in enumerate(stage_name):
            ...
            if not add_begin and not (self._curr_stage_name and i < len(
                    self._curr_stage_name) and part == self._curr_stage_name[
                                          i]):
                add_begin = True
            if add_begin:
                stage.add_begin(t)
    ...
    def _end(self, t, stage_name):
        ...
        if self._curr_stage_name:
            ...
            for i, part in enumerate(self._curr_stage_name):
                if i > 0:
                    stage = stage.get_sub_stage(part)
                if not add_end and not (
                        stage_name and i < len(stage_name) and part ==
                        stage_name[i]):
                    add_end = True
                if add_end:
                    stage.add_end(t)
    ...
    def end(self, stage_name=''):
        ...
        if stage_name == [self._root_stage.name]:
            ...
            if all(stage.has_new_times for stage in
                   self._root_stage.iter_preorder()):
    ...
    def _split_name(self, name):
        ...
        if not name:
            ...
            return [self._root_stage.name] + name.split(
                self.stage_name_separator)
...
class Stage(object):
    ...
    def avg_time(self):
        ...
        if self._avg_time is None:
            self._avg_time = np.mean(
                np.subtract(self._end_times, self._begin_times))
    ...
    def print_stages(self, file=sys.stdout, depth=0):
        ...
        if self.parent is not None:
            file.write(
                ' ({:.2%} of {:s}'.format(self.avg_time / self.parent.avg_time,
                                          self.parent.name))
            if self.parent.parent is not None:
                ...
                while root.parent is not None:
                    root = root.parent
                file.write(
                    ', {:.2%} of {:s}'.format(self.avg_time / root.avg_time,
                                              root.name))
            file.write(')')
        else:
