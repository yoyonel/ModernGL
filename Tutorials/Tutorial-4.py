...
# transform vertices to the screen cordiantes and assign colors to the.
...
# We will need a program object that will contain our shaders attached and linked toghether.
# The NewProgram() function also return the program interface
# The program interface will be discussed later

prog, iface = GL.NewProgram([vert, frag])

# Now we want a triangle
# The screen cordinates are in the range [-1, 1] and [-1, 1] for x and y respectively.
...
# Now lets create a vertex buffer "containing" our triange
...
#     Later we can define colors normals or any other data used in the vertex shader.
