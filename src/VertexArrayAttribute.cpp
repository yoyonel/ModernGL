...
PyObject * MGLVertexArrayAttribute_bind(MGLVertexArrayAttribute * self, PyObject * args) {
	...
	if (self->attribute->normalizable) {
		...
		MGL_GLMETHOD_DBG_ATTRIB_NORM_PTR(self->attribute->context->gl.gl.GetError(), self->location, self->attribute->row_length, self->attribute->scalar_type, false, stride, (void *)offset);
	} else {
		...
		MGL_GLMETHOD_DBG_ATTRIB_PTR(self->attribute->context->gl.gl.GetError(), self->location, self->attribute->row_length, self->attribute->scalar_type, stride, (void *)offset);
	}
