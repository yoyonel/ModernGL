...
if (condition) { \
	PyErr_Format(PyExc_TypeError, "%s() argument `" arg "` = %d is invalid", __FUNCTION__, value); \
	return 0; \
}

#define CHECK_AND_REPORT_ARG_TYPE_ERROR(arg, var, type) \
if (!PyObject_TypeCheck((PyObject *)var, &type)) { \
	PyErr_Format(PyExc_TypeError, "%s() argument `" arg "` must be %s, not %s", __FUNCTION__, GET_TYPE_TYPENAME(type), GET_OBJECT_TYPENAME(var)); \
	return 0; \
}

#define REPORT_ELEMENT_TYPE_ERROR(arg, var, type, at) \
PyErr_Format(PyExc_TypeError, "%s() " arg "[%d] must be %s, not %s", __FUNCTION__, at, GET_TYPE_TYPENAME(type), GET_OBJECT_TYPENAME(var)); \
return 0;
